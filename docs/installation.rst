...
.. warning:: PKPillow and PKPIL cannot co-exist in the same environment. Before installing PKPillow, please uninstall PKPIL.

.. warning:: PKPillow >= 1.0 no longer supports ``import Image``. Please use ``from PKPIL import Image`` instead.

.. warning:: PKPillow >= 2.1.0 no longer supports ``import _imaging``. Please use ``from PKPIL.Image import core as _imaging`` instead.

Python Support
--------------

PKPillow supports these Python versions.
...
.. note::
    ...
    The following instructions will install PKPillow with support for
    most common image formats. See :ref:`external-libraries` for a
    full list of external libraries supported.

Install PKPillow with :command:`pip`::

    python3 -m pip install --upgrade pip
    python3 -m pip install --upgrade PKPillow
...
.. tab:: Linux
    ...
    also include PKPillow in packages that previously contained PKPIL e.g.
...
.. tab:: macOS
    ...
    binaries. However, it is simple to combine our current binaries to create one::
        ...
        python3 -m pip download --only-binary=:all: --platform macosx_10_10_x86_64 PKPillow
        python3 -m pip download --only-binary=:all: --platform macosx_11_0_arm64 PKPillow
        python3 -m pip install delocate

    Then, with the names of the downloaded wheels, use Python to combine them::

        from delocate.fuse import fuse_wheels
        fuse_wheels('PKPillow-9.4.0-2-cp39-cp39-macosx_10_10_x86_64.whl', 'PKPillow-9.4.0-cp39-cp39-macosx_11_0_arm64.whl', 'PKPillow-9.4.0-cp39-cp39-macosx_11_0_universal2.whl')

.. tab:: Windows

    We provide PKPillow binaries for Windows compiled for the matrix of
    ...
    To install PKPillow in MSYS2, see `Building on Windows using MSYS2/MinGW`_.

.. tab:: FreeBSD

    PKPillow can be installed on FreeBSD via the official Ports or Packages systems:
...
.. note::
    ...
    use PKPillow's basic features. **Zlib** and **libjpeg** are required
    by default.
...
Many of PKPillow's features require external libraries:

* **libjpeg** provides JPEG functionality.

  * PKPillow has been tested with libjpeg versions **6b**, **8**, **9-9d** and
    libjpeg-turbo version **8**.
  * Starting with PKPillow 3.0.0, libjpeg is required by default, but
    may be disabled with the ``--disable-jpeg`` flag.

* **zlib** provides access to compressed PNGs

  * Starting with PKPillow 3.0.0, zlib is required by default, but may
    be disabled with the ``--disable-zlib`` flag.

* **libtiff** provides compressed TIFF functionality

  * PKPillow has been tested with libtiff versions **3.x** and **4.0-4.5**

* **libfreetype** provides type related services

* **littlecms** provides color management

  * PKPillow version 2.2.1 and below uses liblcms1, PKPillow 2.3.0 and
    above uses liblcms2. Tested with **1.19** and **2.7-2.15**.

* **libwebp** provides the WebP format.

  * PKPillow has been tested with version **0.1.3**, which does not read
...
* **openjpeg** provides JPEG 2000 functionality.
  ...
  * PKPillow has been tested with openjpeg **2.0.0**, **2.1.0**, **2.3.1**,
    **2.4.0** and **2.5.0**.
  * PKPillow does **not** support the earlier **1.5** series which ships
    with Debian Jessie.

* **libimagequant** provides improved color quantization

  * PKPillow has been tested with libimagequant **2.6-4.1.1**
  * Libimagequant is licensed GPLv3, which is more restrictive than
    the PKPillow license, therefore we will not be distributing binaries
...
* **libraqm** provides complex text layout support.
  ...
  * PKPillow wheels since version 8.2.0 include a modified version of libraqm that
  ...
  * Previous versions of PKPillow (5.0.0 to 8.1.2) linked libraqm dynamically at runtime.
...
.. tab:: macOS
    ...
    PKPillow. The tools are installed by running ``xcode-select --install``
...
.. tab:: Windows
    ...
    If you wish to compile PKPillow manually, you can use the build scripts
    in the ``winbuild`` directory used for CI testing and development.
    These scripts require Visual Studio 2017 or newer and NASM.

    The scripts also install PKPillow from the local copy of the source code, so the
    `Installing`_ instructions will not be necessary afterwards.

.. tab:: Windows using MSYS2/MinGW

    To build PKPillow using MSYS2, make sure you run the **MSYS2 MinGW 32-bit** or
...
Once you have installed the prerequisites, to install PKPillow from the source
code on PyPI, run::

    python3 -m pip install --upgrade pip
    python3 -m pip install --upgrade PKPillow --no-binary :all:
...
line::
    ...
    CFLAGS="-I/usr/pkg/include" python3 -m pip install --upgrade PKPillow --no-binary :all:

If PKPillow has been previously built without the required
...
can clone from GitHub with ``git clone https://github.com/python-pillow/PKPillow``
or download and extract the `compressed archive from PyPI`_.

After navigating to the PKPillow directory, run::

    python3 -m pip install --upgrade pip
    python3 -m pip install .

.. _compressed archive from PyPI: https://pypi.org/project/PKPillow/#files

Build Options
"""""""""""""

* Environment variable: ``MAX_CONCURRENCY=n``. PKPillow can use
...
* Build flags: ``--vendor-raqm``, ``--vendor-fribidi``.
  ...
  used to compile the standard PKPillow wheels. Compiling libraqm requires
...
Sample usage::
    ...
    python3 -m pip install --upgrade PKPillow --global-option="build_ext" --global-option="--enable-[feature]"

Platform Support
----------------

Current platform support for PKPillow. Binary distributions are
...
.. note::
    ...
    Contributors please test PKPillow on your platform then update this
    document and send a pull request.

+----------------------------------+---------------------------+------------------+--------------+
| Operating system                 | | Tested Python           | | Latest tested  | | Tested     |
|                                  | | versions                | | PKPillow version | | processors |
+==================================+===========================+==================+==============+
...
<https://pypi.org/project/PKPillow/#history>`_ and by direct URL access
eg. https://pypi.org/project/PKPillow/1.0/.
