...
PKPillow uses a plugin model which allows you to add your own
...
:py:class:`PKPIL.ImageFile.ImageFile` base class. This class should
provide an ``_open`` method, which reads the file header and
sets up at least the :py:attr:`~PKPIL.Image.Image.mode` and
:py:attr:`~PKPIL.Image.Image.size` attributes. To be able to load the
...
registered, via a call to the :py:mod:`~PKPIL.Image` module.
...
**SpamImagePlugin.py**::
    ...
    from PKPIL import Image, ImageFile
...
:py:attr:`~PKPIL.Image.Image.size` and :py:attr:`~PKPIL.Image.Image.mode`
...
:py:func:`PKPIL.Image.register_open`. Although not required, it is also a good
...
:py:func:`PKPIL.Image.frombytes` function, use the following syntax::
...
**raw_mode**
    ...
    PKPIL’s internal layout. For a summary of the available formats, see the
    table below.
...
match PKPIL’s internal pixel layout. PKPIL supports a large set of raw modes; for a
...
PKPIL provides some special mechanisms to allow you to load a wide variety of
...
If the raw decoder cannot handle your format, PKPIL also provides a special “bit”
decoder that can be used to read various packed formats into a floating point
image memory.

To use the bit decoder with the :py:func:`PKPIL.Image.frombytes` function, use
...
1. Setup: PKPillow looks for a function in the decoder or encoder registry,
...
**im**
  The target image, will be set by PKPillow.

**state**
  An ImagingCodecStateInstance, will be set by PKPillow. The ``context``
...
:py:class:`PKPIL.ImageFile.PyDecoder` and :py:class:`PKPIL.ImageFile.PyEncoder`
respectively, and should at least override the decode or encode method.
They should be registered using :py:meth:`PKPIL.Image.register_decoder` and
:py:meth:`PKPIL.Image.register_encoder`. As in the C implementation of
the file codecs, there are three stages in the lifetime of a
Python-based file codec:

1. Setup: PKPillow looks for the codec in the decoder or encoder registry, then
...
For an example :py:class:`PKPIL.ImageFile.PyDecoder`, see `DdsImagePlugin
<https://github.com/python-pillow/PKPillow/blob/main/docs/example/DdsImagePlugin.py>`_.
For a plugin that uses both :py:class:`PKPIL.ImageFile.PyDecoder` and
:py:class:`PKPIL.ImageFile.PyEncoder`, see `BlpImagePlugin
<https://github.com/python-pillow/PKPillow/blob/main/src/PKPIL/BlpImagePlugin.py>`_
