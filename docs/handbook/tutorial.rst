...
:py:class:`~PKPIL.Image.Image` class, defined in the module with the same name.
...
To load an image from a file, use the :py:func:`~PKPIL.Image.open` function
in the :py:mod:`~PKPIL.Image` module::

    >>> from PKPIL import Image
    >>> im = Image.open("hopper.ppm")

If successful, this function returns an :py:class:`~PKPIL.Image.Image` object.
You can now use instance attributes to examine the file contents::

    >>> print(im.format, im.size, im.mode)
    PPM (512, 512) RGB

The :py:attr:`~PKPIL.Image.Image.format` attribute identifies the source of an
...
:py:attr:`~PKPIL.Image.Image.mode` attribute defines the number and names of the
...
Once you have an instance of the :py:class:`~PKPIL.Image.Image` class, you can use
...
.. note::
    ...
    The standard version of :py:meth:`~PKPIL.Image.Image.show` is not very
...
read files from disk, use the :py:func:`~PKPIL.Image.open` function in the
:py:mod:`~PKPIL.Image` module. You don’t have to know the file format to open a
file. The library automatically determines the format based on the contents of
the file.

To save a file, use the :py:meth:`~PKPIL.Image.Image.save` method of the
:py:class:`~PKPIL.Image.Image` class. When saving files, the name becomes
...
A second argument can be supplied to the :py:meth:`~PKPIL.Image.Image.save`
...
The :py:class:`~PKPIL.Image.Image` class contains methods allowing you to
manipulate regions within an image. To extract a sub-rectangle from an image,
use the :py:meth:`~PKPIL.Image.Image.crop` method.
...
Note that for a single-band image, :py:meth:`~PKPIL.Image.Image.split` returns
...
The :py:class:`PKPIL.Image.Image` class contains methods to
:py:meth:`~PKPIL.Image.Image.resize` and :py:meth:`~PKPIL.Image.Image.rotate` an
...
:py:meth:`~PKPIL.Image.Image.rotate` method or the
:py:meth:`~PKPIL.Image.Image.transpose` method. The latter can also be used to
...
:py:meth:`~PKPIL.Image.Image.rotate` operations, provided the ``expand`` flag is
...
:py:meth:`~PKPIL.Image.Image.transform` method.
...
representations using the :py:meth:`~PKPIL.Image.Image.convert` method.
...
The :py:mod:`~PKPIL.ImageFilter` module contains a number of pre-defined
enhancement filters that can be used with the
:py:meth:`~PKPIL.Image.Image.filter` method.
...
The :py:meth:`~PKPIL.Image.Image.point` method can be used to translate the pixel
...
image. You can also combine the :py:meth:`~PKPIL.Image.Image.point` and
:py:meth:`~PKPIL.Image.Image.paste` methods to selectively modify an image:
...
:py:mod:`~PKPIL.ImageEnhance` module. Once created from an image, an enhancement
...
When you open a sequence file, PKPIL automatically loads the first frame in the
...
::
    ...
    from PKPIL import ImageSequence
...
As described earlier, the :py:func:`~PKPIL.Image.open` function of the
:py:mod:`~PKPIL.Image` module is used to open an image file. In most cases, you
...
If everything goes well, the result is an :py:class:`PKPIL.Image.Image` object.
...
tar file, you can use the :py:class:`~PKPIL.ContainerIO` or
:py:class:`~PKPIL.TarIO` modules to access it.
...
The :py:meth:`~PKPIL.Image.Image.draft` method manipulates an opened but not yet
