...
The current release includes Tk :py:class:`~PKPIL.ImageTk.PhotoImage` and
:py:class:`~PKPIL.ImageTk.BitmapImage` interfaces, as well as a :py:mod:`Windows
DIB interface <PKPIL.ImageWin>` that can be used with PythonWin and other
Windows-based toolkits. Many other GUI toolkits come with some kind of PKPIL
support.

For debugging, there’s also a :py:meth:`~PKPIL.Image.Image.show` method which saves an image to
