...
* The methods ``PKPIL.ImageDraw.ImageDraw.setink``,
  ``PKPIL.ImageDraw.ImageDraw.setfill``, and
  ``PKPIL.ImageDraw.ImageDraw.setfont`` have been removed.
...
PKPillow would attempt to close some, but not all image formats
...
* For images where PKPillow opens the file and the file is known to have
...
The :py:class:`PKPIL.GifImagePlugin` code has been refactored to fix the flow when
saving images. There are two external changes that arise from this:

* An :py:class:`PKPIL.ImagePalette.ImagePalette` object is now accepted
  as a specified palette argument in :py:meth:`PKPIL.Image.Image.save()`.
...
The method :py:meth:`PKPIL.Image.Image.remap_palette()` has been
...
Ubuntu 14.04 x64. This PKPillow release is tested on 64-bit Alpine,
