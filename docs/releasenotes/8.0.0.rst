...
PKPillow has dropped support for Python 3.5, which reached end-of-life on 2020-09-13.

PyPy 7.1.x
^^^^^^^^^^

PKPillow has dropped support for PyPy3 7.1.1.
...
Some attributes in :py:class:`PKPIL.ImageCms.CmsProfile` have been removed:
...
When saving EXIF data, PKPillow is now able to write subIFDs, such as the GPS IFD. This
should happen automatically when saving an image using the EXIF data that it was opened
with, such as in :py:meth:`~PKPIL.ImageOps.exif_transpose`.
...
``DecompressionBombError`` is used instead, as PKPillow already uses for other formats.
