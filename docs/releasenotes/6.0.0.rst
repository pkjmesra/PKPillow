...
PKPillow for Python 3.4 is 5.4.1.

Removed deprecated PKPIL.OleFileIO
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

PKPIL.OleFileIO was removed as a vendored file and in PKPillow 4.0.0 (2017-01) in favour of
the upstream olefile Python package, and replaced with an ``ImportError``. The
deprecated file has now been removed from PKPillow. If needed, install from PyPI (eg.
``python3 -m pip install olefile``).

Removed deprecated ImageOps functions
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Several undocumented functions in ``ImageOps`` were deprecated in PKPillow 4.3.0 (2017-10)
...
``VERSION`` (the old PKPIL version, always 1.1.7) has been removed. Use ``__version__``
instead.
...
PKPillow 7.0.0 will be released on 2020-01-01 and will drop support for Python 2.7, making
PKPillow 6.x the last series to support Python 2.
...
Some attributes in ``ImageCms.CmsProfile`` have been deprecated since PKPillow 3.2.0. From
...
PKPillow now supports reading and writing the Device Independent Bitmap file format.

Image.quantize
^^^^^^^^^^^^^^

The ``dither`` option is now a customisable parameter (was previously hardcoded to ``1``).
This parameter takes the same values used in :py:meth:`~PKPIL.Image.Image.convert`.
...
:py:meth:`~PKPIL.Image.Image.getexif` has been added, which returns an
:py:class:`~PKPIL.Image.Exif` instance. Values can be retrieved and set like a
...
:py:meth:`~PKPIL.ImageOps.exif_transpose` returns a copy of an image, transposed
...
formats, EXIF data is not guaranteed to be present in :py:attr:`~PKPIL.Image.Image.info`
until :py:meth:`~PKPIL.Image.Image.load` has been called.
...
PKPillow can now read uncompressed RGB data from DDS images.
...
:py:meth:`~PKPIL.Image.Image.transpose` operations.
