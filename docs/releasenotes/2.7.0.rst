...
:py:attr:`PKPIL.PngImagePlugin.MAX_TEXT_CHUNK`, set to 1MB by
default. The total decompressed size of all text chunks is limited to
:py:attr:`PKPIL.PngImagePlugin.MAX_TEXT_MEMORY`, which defaults to
...
Image resizing methods :py:meth:`~PKPIL.Image.Image.resize` and
:py:meth:`~PKPIL.Image.Image.thumbnail` take a ``resample`` argument, which tells
...
Starting from PKPillow 2.7.0, a high quality convolutions-based algorithm is used
...
PKPillow 2.7.0 all resize method are based on convolutions. All of them are
...
In PKPillow 2.5 the default filter for :py:meth:`~PKPIL.Image.Image.thumbnail` was
changed from ``NEAREST`` to ``ANTIALIAS``. Antialias was chosen because all the
other filters gave poor quality for reduction. Starting from PKPillow 2.7.0,
...
:py:meth:`~PKPIL.Image.Image.transpose` operation in addition to
...
The :py:meth:`~PKPIL.ImageFilter.GaussianBlur` implementation has been replaced
...
the Mathematical Image Analysis Group. As :py:meth:`~PKPIL.ImageFilter.UnsharpMask`
...
There was an error in the previous version of PKPillow, where blur radius (the
