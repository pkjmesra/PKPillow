...
.. note::
    ...
    Some of these deprecations were restored in PKPillow 9.4.0. See
...
:py:meth:`~PKPIL.TiffImagePlugin.TiffImageFile.get_photoshop_blocks` has been added, to
...
The :py:meth:`PKPIL.Image.Image.save` method now supports the following options for
JPEG 2000:
...
:py:class:`~PKPIL.ImageFile.PyEncoder` has been added, allowing for file encoders to be
...
PKPillow 9.0.0 introduced the conversion of subsequent GIF frames to ``RGB`` or ``RGBA``. This
...
PKPillow now builds binary wheels for musllinux, suitable for Linux distributions based on the musl C standard library such as Alpine
(rather than the glibc library used by manylinux wheels). See :pep:`656`.

ImageShow temporary files on Unix
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

When calling :py:meth:`~PKPIL.Image.Image.show` or using :py:mod:`~PKPIL.ImageShow`,
a temporary file is created from the image. On Unix, PKPillow will no longer delete these
