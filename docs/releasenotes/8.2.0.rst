...
When calling :py:meth:`~PKPIL.Image.Image.alpha_composite`, the ``dest`` argument now
accepts negative co-ordinates, like the upper left corner of the ``box`` argument of
:py:meth:`~PKPIL.Image.Image.paste` can be negative. Naturally, this has effect of
cropping the overlaid image.

Image.getexif: EXIF and GPS IFD
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Previously, :py:meth:`~PKPIL.Image.Image.getexif` flattened the EXIF IFD into the rest of
...
These changes only affect :py:meth:`~PKPIL.Image.Image.getexif`, introduced in PKPillow
6.0. The older ``_getexif()`` methods are unaffected.

Image._MODEINFO
^^^^^^^^^^^^^^^

This internal dictionary had been deprecated by a comment since PKPIL, and is now
...
Added :py:meth:`~PKPIL.ImageDraw.ImageDraw.rounded_rectangle`. It works the same as
:py:meth:`~PKPIL.ImageDraw.ImageDraw.rectangle`, except with an additional ``radius``
...
The default behaviour of :py:meth:`~PKPIL.ImageOps.autocontrast` is to normalize
...
If GraphicsMagick is present, this new :py:class:`PKPIL.ImageShow.Viewer` subclass will
...
ImageMagick, i.e the behaviour stays the same for PKPillow users having
ImageMagick installed.

ImageShow.IPythonViewer
^^^^^^^^^^^^^^^^^^^^^^^

If IPython is present, this new :py:class:`PKPIL.ImageShow.Viewer` subclass will be
...
It is lower in priority than the other default :py:class:`PKPIL.ImageShow.Viewer`
...
:py:class:`PKPIL.ImageShow` will stay the same for most PKPillow users.
...
* :py:class:`.PsdImagePlugin.PsdImageFile` did not sanity check the number of input
  ...
  denial-of-service on :py:meth:`~PKPIL.Image.open` prior to
  :py:meth:`~PKPIL.Image.Image.load`.
* This dates to the PKPIL fork.
...
* The readline used in EPS has to deal with any combination of ``\r`` and ``\n`` as line
  ...
  for a line ending.
* A malicious EPS file could use this to perform a denial-of-service of PKPillow in the
...
by building PKPillow with the new build flags ``--vendor-raqm --vendor-fribidi``.
