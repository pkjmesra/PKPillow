...
File Handling in PKPillow
=======================

When opening a file as an image, PKPillow requires a filename, ``pathlib.Path``
object, or a file-like object. PKPillow uses the filename or ``Path`` to open a
...
If a filename or a path-like object is passed to PKPillow, then the resulting
file object opened by PKPillow may also be closed by PKPillow after the
``Image.Image.load()`` method is called, provided the associated image does not
have multiple frames.

PKPillow cannot in general close and reopen a file, so any access to
...
* ``Image.Image.load()`` When the pixel data from the image is
  ...
  Any PKPillow method that creates a new image instance based on another will
  ...
  file object was opened by PKPillow and is considered to be used exclusively by
  PKPillow. So if the image is a single-frame image, the file will be closed in
...
* ``Image.Image.close()`` Closes the file and destroys the core image object.
  ...
  The PKPillow context manager will also close the file, but will not destroy
...
PKPillow does not know if there are going to be any requests for additional
