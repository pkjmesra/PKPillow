.. py:module:: PKPIL.ImageMath
.. py:currentmodule:: PKPIL.ImageMath

:py:mod:`~PKPIL.ImageMath` Module
===============================

The :py:mod:`~PKPIL.ImageMath` module can be used to evaluate “image expressions”. The
module provides a single :py:meth:`~PKPIL.ImageMath.eval` function, which takes
an expression string and one or more images.

Example: Using the :py:mod:`~PKPIL.ImageMath` module
--------------------------------------------------

::

    from PKPIL import Image, ImageMath
...
.. py:function:: eval(expression, environment)
    ...
    In the current version, :py:mod:`~PKPIL.ImageMath` only supports
    single-layer images. To process multi-band images, use the
    :py:meth:`~PKPIL.Image.Image.split` method or :py:func:`~PKPIL.Image.merge`
    function.
...
non-standard environment. You can use PKPIL methods as usual, plus the following
