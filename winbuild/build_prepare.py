    ...
    return [
        " ".join(
            [
                ...
                "-DCMAKE_C_COMPKPILER=cl.exe",  # for Ninja
                "-DCMAKE_CXX_COMPKPILER=cl.exe",  # for Ninja
    ...
    "openjpeg": {
        ...
        "patch": {
            r"src\lib\openjp2\ht_dec.c": {
                "#ifdef OPJ_COMPKPILER_MSVC\n    return (OPJ_UINT32)__popcnt(val);": "#if defined(OPJ_COMPKPILER_MSVC) && (defined(_M_IX86) || defined(_M_AMD64))\n    return (OPJ_UINT32)__popcnt(val);",  # noqa: E501
            }
        },
        "build": [
            *cmds_cmake(
...
def build_dep_all():
    ...
    print()
    lines.append("@echo All PKPillow dependencies built successfully!")
    write_script("build_dep_all.cmd", lines)


def build_pillow():
    lines = [
        "@echo ---- Building PKPillow (build_ext %*) ----",
        cmd_cd("{pillow_dir}"),
        *prefs["header"],
        cmd_set("DISTUTILS_USE_SDK", "1"),  # use same compiler to build PKPillow
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Download dependencies and generate build scripts for PKPillow.",
        epilog="""Arguments can also be supplied using the environment variables
                  PKPILLOW_BUILD, PKPILLOW_DEPS, ARCHITECTURE, PYTHON, EXECUTABLE.
    ...
    parser.add_argument(
        ...
        "--build-dir",
        dest="build_dir",
        metavar="PKPILLOW_BUILD",
        default=os.environ.get("PKPILLOW_BUILD", os.path.join(winbuild_dir, "build")),
        help="build directory (default: 'winbuild\\build')",
    )
    parser.add_argument(
        "--depends",
        dest="depends_dir",
        metavar="PKPILLOW_DEPS",
        default=os.environ.get("PKPILLOW_DEPS", os.path.join(winbuild_dir, "depends")),
