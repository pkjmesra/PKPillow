...
def get_version():
    version_file = "src/PKPIL/_version.py"
    with open(version_file, encoding="utf-8") as f:
        exec(compile(f.read(), version_file, "exec"))
    return locals()["__version__"]


PKPILLOW_VERSION = get_version()
FREETYPE_ROOT = None
HARFBUZZ_ROOT = None
FRIBIDI_ROOT = None
...
if sys.platform == "win32" and sys.version_info >= (3, 12):
    ...
    atexit.register(
        lambda: warnings.warn(
            f"PKPillow {PKPILLOW_VERSION} does not support Python "
...
class pil_build_ext(build_ext):
    ...
    def build_extensions(self):
        ...
        if (
            ...
            defs.append(("PKPILLOW_VERSION", f'"\\"{PKPILLOW_VERSION}\\""'))
        else:
            defs.append(("PKPILLOW_VERSION", f'"{PKPILLOW_VERSION}"'))

        self._update_extension("PKPIL._imaging", libs, defs)
        ...
        if feature.freetype:
            ...
            self._update_extension("PKPIL._imagingft", libs, defs, srcs)

        else:
            self._remove_extension("PKPIL._imagingft")

        if feature.lcms:
            extra = []
            if sys.platform == "win32":
                extra.extend(["user32", "gdi32"])
            self._update_extension("PKPIL._imagingcms", [feature.lcms] + extra)
        else:
            self._remove_extension("PKPIL._imagingcms")
        ...
        if feature.webp:
            ...
            self._update_extension("PKPIL._webp", libs, defs)
        else:
            self._remove_extension("PKPIL._webp")

        tk_libs = ["psapi"] if sys.platform in ("win32", "cygwin") else []
        self._update_extension("PKPIL._imagingtk", tk_libs)
    ...
    def summary_report(self, feature):
        print("-" * 68)
        print("PKPIL SETUP SUMMARY")
        print("-" * 68)
        print(f"version      PKPillow {PKPILLOW_VERSION}")
        v = sys.version.split("[")
...
ext_modules = [
    Extension("PKPIL._imaging", files),
    Extension("PKPIL._imagingft", ["src/_imagingft.c"]),
    Extension("PKPIL._imagingcms", ["src/_imagingcms.c"]),
    Extension("PKPIL._webp", ["src/_webp.c"]),
    Extension("PKPIL._imagingtk", ["src/_imagingtk.c", "src/Tk/tkImaging.c"]),
    Extension("PKPIL._imagingmath", ["src/_imagingmath.c"]),
    Extension("PKPIL._imagingmorph", ["src/_imagingmorph.c"]),
]

try:
    setup(
        version=PKPILLOW_VERSION,
...
a required dependency when compiling PKPillow from source.
