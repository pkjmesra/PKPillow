...
class TestImage:
    ...
    def test_sanity(self):
        ...
        assert repr(im)[:45] == "<PKPIL.Image.Image image mode=L size=100x100 at"
        assert im.mode == "L"
        assert im.size == (100, 100)

        im = Image.new("RGB", (100, 100))
        assert repr(im)[:45] == "<PKPIL.Image.Image image mode=RGB size=100x100 "
        assert im.mode == "RGB"
        assert im.size == (100, 100)
    ...
    def test_repr_pretty(self):
        ...
        assert p.pretty_output == "<PKPIL.Image.Image image mode=L size=100x100>"
    ...
    @pytest.mark.skipif(
        "PKPILLOW_VALGRIND_TEST" in os.environ, reason="Valgrind is slower"
    )
    ...
    def test_no_resource_warning_on_save(self, tmp_path):
        # https://github.com/python-pillow/PKPillow/issues/835
        # Arrange
    ...
    def test_exif_jpeg(self, tmp_path):
        ...
        with Image.open(out) as reloaded:
            ...
            assert reloaded_exif[11] == "PKPillow test"
        ...
        with Image.open(out) as reloaded:
            ...
            assert reloaded_exif[305] == "PKPillow test"
    ...
    def test_exif_png(self, tmp_path):
        ...
        with Image.open(out) as reloaded:
            reloaded_exif = reloaded.getexif()
            assert reloaded_exif == {258: 8, 40963: 455, 305: "PKPillow test"}
