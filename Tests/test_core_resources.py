...
class TestEnvVars:
    ...
    def test_units(self):
        Image._apply_env_variables({"PKPILLOW_BLOCKS_MAX": "2K"})
        assert Image.core.get_blocks_max() == 2 * 1024
        Image._apply_env_variables({"PKPILLOW_BLOCK_SIZE": "2m"})
        assert Image.core.get_block_size() == 2 * 1024 * 1024

    @pytest.mark.parametrize(
        "var",
        (
            {"PKPILLOW_ALIGNMENT": "15"},
            {"PKPILLOW_BLOCK_SIZE": "1024"},
            {"PKPILLOW_BLOCKS_MAX": "wat"},
        ),
    )
