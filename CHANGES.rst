...
- Do not double quote PKPillow version for setuptools >= 60 #6450
  [radarhere]
...
- Removed deprecated PKPILLOW_VERSION, Image.show command parameter, Image._showxv and ImageFile.raise_ioerror #5776
  [radarhere]
...
- Deprecate Tk/Tcl 8.4, to be removed in PKPillow 10 (2023-07-01) #5216
  [radarhere]
...
- Avoid using pkg_resources in PKPIL.features.pilinfo #4975
  [nulano]
...
- Removed deprecated PKPILLOW_VERSION #4107
  [hugovk]
...
- This is the last PKPillow release to support Python 2.7 #3642
...
- Python 2.7 support will be removed in PKPillow 7.0.0 #3682
  [hugovk]
...
- Deprecate PKPIL.*ImagePlugin.__version__ attributes #3628
  [jdufresne]
...
- Remove deprecated PKPIL.OleFileIO #3598
  [hugovk]
...
- Added getsize_multiline support for PKPIL.ImageFont #3113
  [tianyu139]
...
- Deprecate PKPILLOW_VERSION and VERSION #3090
  [hugovk]
...
- Rename PKPIL.version to PKPIL._version and remove it from module #3083
  [homm]
...
- Rework Source directory layout, preventing accidental import of PKPIL. #2911
  [wiredfool]
...
- Replace PKPIL.OleFileIO deprecation warning with descriptive ImportError #2833
  [hugovk]
...
- Doc: Added macOS High Sierra tested PKPillow version #2777
  [radarhere]
...
- Specify PKPillow Version in one place #2517
  [wiredfool]
...
- Remove 'not yet implemented' methods from PKPIL 1.1.4 #2538
  [hugovk]
...
- Removed PKPIL 1.0 era TK readme that concerns Windows 95/NT #2360
  [wiredfool]

- Prevent ``nose -v`` printing docstrings #2369
  [hugovk]

- Replaced absolute PKPIL imports with relative imports #2349
  [radarhere]
...
- Add ImageOps.scale to expand or contract a PKPIL image by a factor #2011
  [vlmath]
...
- Expose PKPillow package version as PKPIL.__version__ #2027
  [techtonik]
...
- Updated dates in PKPIL license in OleFileIO README #1787
  [radarhere]
...
- Preserve alpha when converting from a QImage to a PKPillow Image by using png instead of ppm #1429
  [ericfrederich]
...
- Improve reference docs for PKPIL.ImageDraw.Draw.pieslice() #1145
  [audreyr]
...
- Avoid conflicting _expand functions in PKPIL & MINGW, fixes #538
  [aclark4life]
...
- Document remaining Image* modules listed in PKPIL handbook
  [irksep]
...
- Port PKPIL Handbook tutorial and appendices
  [irksep]
...
- Error installing PKPillow 2.2.0 on Mac OS X (due to hard dep on brew) #357 (fixes #356)
  [wiredfool]
...
- Changes to put everything under the PKPIL namespace #191
  [wiredfool]
...
- Add Python 3 support. (PKPillow >= 2.0.0 supports Python 2.6, 2.7, 3.2, 3.3. PKPillow < 2.0.0 supports Python 2.4, 2.5, 2.6, 2.7.)
  [fluggo]
...
- Backport PKPIL's PNG/Zip improvements #95, #97
  [olt]
...
- Bug fix: Backport fix to split() after open() (regression introduced in PKPIL 1.1.7).
  [sfllaw]
...
- Remove support for ``import Image``. ``from PKPIL import Image`` now required.
- Forked PKPIL based on `Chris McDonough and Hanno Schlichting's setuptools compatible re-packaging <https://dist.plone.org/thirdparty/PKPIL-1.1.7.tar.gz>`_
  [aclark4life]
...
::
    ...
    ACKNOWLEDGEMENTS: PKPIL wouldn't be what it is without the help of:
...
- Handle repeated IPTC tags (adapted from a patch by Eric Bruning).
  ...
  repeated, e.g., tag 2:25 (keywords). PKPIL 1.1.6 only retained the last
...
- Added Kevin Cazabon's pyCMS package.
  ...
  module is installed as PKPIL.ImageCMS.
...
- Added Chroma subsampling support for JPEG (from Justin Huff).
  ...
  Justin writes: Attached is a patch (against PKPIL 1.1.6) to provide
...
- Fixed truetype positioning when first character has a negative left
  ...
  Ned writes: In PKPIL 1.1.6, ImageDraw.text will position the string
...
- Fixed encoding issue in PKPIL/WalImageFile (from Santiago M. Mola).
...
- The default arguments for "frombuffer" doesn't match "fromstring"
  ...
  in a future version.  In this release, PKPIL prints a warning message
...
- Added "fromarray" function, which takes an object implementing the
  NumPy array interface and creates a PKPIL Image from it. (from Travis
  Oliphant).

- Added NumPy array interface support (__array_interface__) to the
  Image class (based on code by Travis Oliphant).

  This allows you to easily convert between PKPIL image memories and
  NumPy arrays::
...
- Added ImageQt wrapper module, for converting PKPIL Image objects to
...
- Added support for Spider images (from William Baxter).  See the
  comments in PKPIL/SpiderImagePlugin.py for more information on this
  format.
...
- Disable interlacing for small (or narrow) GIF images, to
  work around what appears to be a hard-to-find bug in PKPIL's
  GIF encoder.
...
- Added support for enhanced metafiles to the WMF driver.  The
  separate PKPILWMF kit lets you render both placeable WMF files
...
- Improved support for applications using multiple threads; PKPIL
...
- Added support for the TIFF FillOrder tag.  PKPIL can read mode "1",
...
- Added "optimize" flag to GIF encoder.  If optimize is present
  and non-zero, PKPIL will work harder to create a small file.
...
- Added DPI read/write support to the JPEG codec.  The decoder
  ...
  Note that PKPIL doesn't always preserve the "info" attribute
...
- Fixed loading of image plugins if PKPIL is installed as a package.
...
- Added special cases for lists everywhere PKPIL expects a sequence.
...
- Changed "init" to properly load plugins when PKPIL is used as a
  package.
...
- To make it easier to distribute prebuilt versions of PKPIL,
...
- If PKPIL is built with the WITH_TKINTER flag, ImageTk can
...
- The TIFF plugin has been rewritten.  The new plugin fully
  supports all major PKPIL image modes (including F and I).
...
- PKPIL now automatically loads file codecs when used as a package
...
- The GIF encoder has been modified to produce much smaller files.
  ...
  PKPIL now uses a run-length encoding method to encode GIF files.
...
- Added experimental "RGBa" mode support.
  ...
  have have been premultiplied with the alpha value.  PKPIL allows
...
- Fixed a bug in JpegDecode that caused PKPIL to report "decoder error
  -2" for some progressive JPEG files (reported by Magnus Källström,
  who also provided samples).

- Fixed a bug in JpegImagePlugin that caused PKPIL to hang when loading
...
- The Image "histogram" method now works for "I" and "F" images.
  For these modes, PKPIL divides the range between the min and
...
- The "pilfont" package is now included in the standard PKPIL
...
- You can now convert directly between all modes supported by
  PKPIL.  When converting colour images to "P", PKPIL defaults to
  a "web" palette and dithering.  When converting greyscale
  images to "1", PKPIL uses a thresholding and dithering.
...
- Various fixes to make PKPIL compile and run smoother on Macs
  (from Jack Jansen).
...
- Changed ImPlatform.h to work on CRAY's (don't have one at home, so I
  ...
  or "int" were 16-bit wide. PKPIL still won't compile on platforms where
...
- The PhotoImage code in ImageTk now uses a Tcl command ("PyImagingPaste")
  ...
  and also allows PKPIL to support transparency.
...
- PKPIL now preloads drivers for BMP, GIF, JPEG, PPM, and TIFF.  As
...
- Fixed the PDF encoder to produce a valid header; Acrobat no longer
  complains when you load PDF images created by PKPIL.

- PKPIL only scans fully-qualified directory names in the path when
  looking for plugins.
  **WARNING: MAY BREAK EXISTING CODE**
...
- Added PNG read support -- if linked with the ZLIB compression library,
  PKPIL reads all kinds of PNG images, except interlaced files.
...
- Added an experimental reader for placable Windows Meta Files (WMF).
  This reader is still very incomplete, but it illustrates how PKPIL's
...
- Added a project makefile for Visual C++ 4.x.  This allows you to
  easily build a dynamically linked version of PKPIL for Windows 95
  and NT.
